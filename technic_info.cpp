...
Technic_info::pack_as_blob()
{
  ...
  blob_       = reinterpret_cast<BYTE *>(malloc(size + analytic_entity.size() * (sizeof(Sql_rmc_field_analog) + sizeof(Rmc_field_header)) + (fuel.empty() ? 0 : (sizeof(Sql_rmc_field_analog) + sizeof(Rmc_field_header)))));
  ...
  try {
    for (auto const & kv : analytic_entity) {
...
//------------------------------------------------------------------------------
std::ostream & operator<<(std::ostream & os, Technic_info const & ti)
{
  os << "---------------------------------------------\n";
