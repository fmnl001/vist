...
//------------------------------------------------------------------------------
static void
setup_logging(std::string const & log_file_path,
              int log_level)
{
  if (log_level == 6)
  ...
  logging::formatter logfmt = expr::format("%1%|%2%|%3%|%4%\n\t%5%%6%") % fmttimestamp % fmttprocessid % fmtthreadid % fmtseverity % fmtscope % expr::smessage;
    ...
    if ((strcmp(name, "Technic") == 0)) {
      ...
      for (int i = 0; atts[i]; i += 2) {
        if (strcmp(atts[i], "id") == 0) {
          ...
          if (!last_id.compare("741") || !last_id.compare("742") || !last_id.compare("743") || !last_id.compare("744") || !last_id.compare("41") || !last_id.compare("202")) {
                                  ...
                                  void *userp)
{
  ...
  if (XML_Parse(parser, (char const *)contents, real_size, 0) == 0) {
...
//------------------------------------------------------------------------------
void handler1(boost::system::error_code const & error,
